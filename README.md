...
音声認識と読み上げは、端末にインストール済みのオフライン機能だけを使います。モデル未配置時は、画面の「モデルをダウンロード」からHugging Face経由でモデルを取得できます。ダウンロード完了後は推論を端末内で実行します。

## モデル配置

Gemma 4 E2B のモデルファイルはサイズが大きいため、APK とリポジトリには含めません。初回起動時に画面の「モデルをダウンロード」を押すと、端末のアプリ専用モデル領域へ保存されます。
